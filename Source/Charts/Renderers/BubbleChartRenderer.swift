...
open class BubbleChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    private var cirleSize = CGFloat(0.0)
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: IBubbleChartDataSet, dataSetIndex: Int)
    {
        ...
        for j in _xBounds
        {
            ...
            var connection = entry.data as? [String:Any]
            let title = connection?["name"] as? String
            
            var string = "\(title ?? "")" as NSString
            if string.length > 8 {
                string = "\(string.substring(to: 5))..." as NSString
            }
            let paragraphStyle = NSMutableParagraphStyle()
            paragraphStyle.alignment = .center
            let attrs = [NSAttributedString.Key.font: UIFont(name: "WorkSans-Regular", size: 14)!, NSAttributedString.Key.paragraphStyle: paragraphStyle]
            let size = string.size(withAttributes: attrs)
            let textRect = CGRect(x: _pointBuffer.x-max(shapeHalf,size.width/2), y: _pointBuffer.y+(shapeHalf-size.height), width: max(shapeSize,size.width), height: size.height+8)

            nameWidth = textRect.width
            nameHeight = textRect.height
            
            connection?.updateValue(cirleSize, forKey: "size")
            connection?.updateValue(_pointBuffer.x - shapeHalf + textRect.height/2, forKey: "x")
            connection?.updateValue(_pointBuffer.y - shapeHalf + 4, forKey: "y")
            
            entry.data = connection
            
            if shapeSize != 0 && entry.data != nil{
                cirleSize = shapeSize - textRect.height
            }
            
            
            
            let rect = CGRect(
                x: _pointBuffer.x - shapeHalf + textRect.height/2,
                y: _pointBuffer.y - shapeHalf + 4,
                width: cirleSize,
                height: cirleSize
            )
            
            let mainRect = CGRect(x: _pointBuffer.x-shapeHalf, y: _pointBuffer.y-shapeHalf,
...
//                context.fill(textRect)
                ...
                context.setStrokeColor(UIColor(red: 0.122, green: 0.129, blue: 0.141, alpha: 1).cgColor)
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        for i in 0..<dataSets.count
        {
            ...
            for j in _xBounds
            {
                ...
                if let connection = e.data as? [String:Any],let size = connection["normalizedTime"] as? Double {
                    valueFont = valueFont.withSize(CGFloat(max((size*44),16)))
                }
