...
open class ChartHighlighter : NSObject, IHighlighter
{
    ...
    open func getHighlight(x: CGFloat, y: CGFloat) -> Highlight?
    {
        ...
        guard let bubbleChart = chart as? BubbleChartView else {
            return getHighlight(xValue: xVal, x: x, y: y)
        }
        return getBubbleHighlight(xValue: xVal, x: x, y: y)
    ...
    ///   - x:
    ///   - y:
    /// - Returns: The corresponding ChartHighlight for a given x-value and xy-touch position in pixels. for buble chart
    @objc open func getBubbleHighlight(xValue xVal: Double, x: CGFloat, y: CGFloat) -> Highlight?
    {
        guard let chart = chart else { return nil }
        var h:Highlight? = nil
        chart.data?.dataSets.forEach({
            for index in 0...$0.entryCount {
                if let entry = $0.entryForIndex(index), let data = entry.data as? [String:Any]{
                    let pointX = data["x"] as! CGFloat
                    let pointY = data["y"] as! CGFloat
                    let size = data["size"] as! CGFloat
                                    
                    if(x >= pointX && x <= (pointX + size) && y >= pointY && y <= (pointY + size)) {
                        h = Highlight(x: entry.x, y: entry.y, dataSetIndex: 4)
